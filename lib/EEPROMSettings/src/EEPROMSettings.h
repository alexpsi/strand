...
public:
  ...
  EEstore(const T &initial) : EEstore()
  {
    if (isFirstStart())
      *this << initial;
  }
  ...
  {
    if (eeAddr)
      update_block((const uint8_t *)&val, (void *)eeAddr, sizeof(T));
    return *this;
  }
  const EEstore &operator>>(T &val) const
  {
    if (eeAddr)
      read_block((uint8_t *)&val, (void *)eeAddr, sizeof(T));
    return *this;
  }
  const T get() const
  {
    ...
    if (eeAddr)
      read_block(buff, (void *)eeAddr, sizeof(T));
    return *((T *)buff);
  }
};
...
public:
  ...
  const EEstring &operator<<(const char *val) const
  {
    ...
    if (eeAddr)
      update_block((const uint8_t *)val, (void *)eeAddr, len > maxLen ? maxLen : len);
    return *this;
  }
  ...
  const EEstring &operator>>(String &val) const
  {
    char buf[maxLen + 1];
    if (eeAddr)
      read_block((uint8_t *)buf, (void *)eeAddr, maxLen);
    buf[maxLen] = '\0';
    val = String(buf);
    return *this;
  }
